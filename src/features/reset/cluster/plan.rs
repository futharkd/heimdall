...
mod tests {
    ...
    #[test]
    fn plan_contains_expected_order_without_redundant_sudo_in_plan_text() {
        ...
        assert!(
            matches!(
                &plan[0].kind,
                OperationKind::Shell { command, .. } if command == "flux"
            ),
            "executor elevates shell ops; plans use bare flux/kubectl/sh/rm/systemctl"
        );
    }
}
